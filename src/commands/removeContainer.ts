...
export async function removeContainer(context: IActionContext, node: ContainerTreeItem | undefined): Promise<void> {
    ...
    if (node) {
        ...
        nodes = await ext.containersTree.showTreeItemPicker(ContainerTreeItem.allContextValueRegExp, { ...context, canPickMany: true });
    }

    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Removing Container(s)..." }, async () => {
